#!/bin/bash
# SothisAI「模型训练」：FM9G + DGC（对齐 train50/9G/opt-9G.sh）
# 网页启动命令只填这一行：
#   bash /root/private_data/ff/scripts/sothis_launch_fm9g_opt.sh
#
# 核心并行与 baseline 一致：TP=1、PP=10，DP=总卡数/TP/PP。
# Megatron 用 ff/dcu_megatron_opt（在 dcu_megatron 上移植了 train50 的 top-k DGC）。
# 默认 --dgc-enabled --dgc-density 0.01，可用 DGC_DENSITY / DGC_MOMENTUM / DGC_MIN_NUMEL 覆盖。
set -euo pipefail
...
FF="$PRIVATE/ff"
# 默认走带 DGC 的 dcu_megatron_opt。覆盖：MEGATRON_PATH=/root/private_data/ff/dcu_megatron bash ...
MEGATRON_PATH="${MEGATRON_PATH:-$FF/dcu_megatron_opt}"
if [ ! -d "$MEGATRON_PATH" ]; then
  echo "ERROR: 找不到 $MEGATRON_PATH，请确认已生成 ff/dcu_megatron_opt" >&2
  exit 1
fi
...
LOG_ROOT="$FF/logs/fm9g-opt"
DGC_DENSITY="${DGC_DENSITY:-0.01}"
...
if [[ -z "${CHECKPOINT_PATH:-}" ]]; then
  if [[ "$NNODES" -gt 1 ]]; then
    CHECKPOINT_PATH="$FF/checkpoints/fm9g-opt-${NNODES}n"
  else
    CHECKPOINT_PATH="$FF/checkpoints/fm9g-opt"
  fi
fi
...
finalize_parallel() {
  ...
  if (( DP < 2 )); then
    echo "WARNING: DP=$DP < 2，没有数据并行副本，DGC 不会触发稀疏通信。" >&2
  fi
...
echo "megatron=$MEGATRON_PATH entry=$ENTRY_PY dgc_density=$DGC_DENSITY dgc_momentum=$DGC_MOMENTUM dgc_min_numel=$DGC_MIN_NUMEL" | tee -a "$LOG_FILE"
...
# 对齐 train50 opt：TP=1 PP=10、SGD、不开 distributed optimizer、full recompute，外加 DGC。
torchrun \
  --nnodes="$NNODES" \
