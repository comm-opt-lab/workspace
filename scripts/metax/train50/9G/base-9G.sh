...
LOG_ROOT=${BASE_PATH}/log/accept/9G-base/WS${WORLD_SIZE}_TP${TP}_PP${PP}_DP${DP}_L${NUM_LAYERS}_iter${TRAIN_ITERATIONS}
mkdir -p "${LOG_ROOT}"
NODE_TAG=${NODE_RANK}
...
run_one() {
       ...
       if [ "${PROFILE_TORCH_ENABLED}" = "1" ]; then
              local PROFILER_DIR="${BASE_PATH}/profiler/9G-base"
